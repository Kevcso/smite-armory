      ...
      <div className="max-w-6xl mx-auto">
        ...
        {gods.length === 0 ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {gods.map(god => (
              <div key={god.id} className="bg-gray-800 p-6 rounded-lg hover:bg-gray-750 transition">
                <h2 className="text-2xl font-bold">{god.name}</h2>
                <p className="text-gray-400">{god.title}</p>
                <div className="mt-4 flex gap-2">
                  <span className="bg-blue-600 px-3 py-1 rounded text-sm">{god.class}</span>
                  {god.pantheon && (
                    <span className="bg-purple-600 px-3 py-1 rounded text-sm">{god.pantheon}</span>
                  )}
                </div>
              </div>
            ))}
